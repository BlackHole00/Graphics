...
R_Resource_Scope :: enum {
	Static,
}

R_Handle :: bit_field u64 {
	gen: u32 | 32,
	idx: u32 | 24,
	scope: R_Resource_Scope | 8,
}
...
R_Resources :: struct {
	...
	textures:		sa.Small_Array(1024, R_Texture),
...
r_sync_resource_storage_gpu_references :: proc() {
	...
	for i := 0; i < sa.len(r_resources.textures); i += 1 {
		texture := &r_resources.textures.data[i]
		texture_array_ptr[i] = texture.texture->gpuResourceID()
	}
	for i := sa.len(r_resources.textures); i < sa.cap(r_resources.textures); i += 1 {
...
r_register_texture_from_bytes :: proc(
	...
	handle.gen = 0
	handle.scope = .Static
	handle.idx = cast(u32)sa.len(r_resources.textures)

	if sync.mutex_guard(&r_resources.textures_mutex) {
		ok := sa.append(&r_resources.textures, texture)
...
r_get_texture :: proc(handle: R_Texture_Handle) -> (texture: ^R_Texture, ok: bool) #optional_ok {
	...
	texture, ok = sa.get_ptr_safe(&r_resources.textures, cast(int)handle.idx)
