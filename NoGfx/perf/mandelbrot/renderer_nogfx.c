...
void initRenderer(void) {
	...
	unsigned int constants[3] = { WINDOW_WIDTH, WINDOW_HEIGHT, 128 * 1024 };
...
void draw(void) {
	...
	GpuAllocation fragmentArgs = gpuBumpAlloc(&gRenderer.tempDataAllocator, sizeof(DrawVertexArgs));
	((DrawFragmentArgs*)fragmentArgs.cpu)->time = gState.frameCount / 100.0;
	gpuDrawIndexedInstanced(commandBuffer, vertexArgs.gpu, fragmentArgs.gpu, gRenderer.triangleIndices.gpu, 3, 1, &gResult); CHECK_RES();
