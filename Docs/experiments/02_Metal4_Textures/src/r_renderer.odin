...
R_Renderer :: struct {
	...
	texture_select_buffer:	^MTL.Buffer,
...
r_init :: proc(target_view: ^AK.View) {
	...
	r_renderer.argument_table->setAddress(r_renderer.index_buffer->gpuAddress(), 1)
	r_renderer.argument_table->setAddress(r_renderer.texture_select_buffer->gpuAddress(), 2)
	r_renderer.argument_table->setAddress(r_resources.gpu_references->gpuAddress(), 3)
	...
	r_renderer.residency_set->addAllocation(r_renderer.texture_select_buffer)
...
r_prepare_texture_select_buffer :: proc() {
	initial_textures := [3]u32{ 1, 1, 1 }

	r_renderer.texture_select_buffer = r_renderer.device->newBufferWithBytes(
		raw_data(initial_textures[:]),
		size_of(initial_textures),
		{ .HazardTrackingModeUntracked },
	)
}

r_setup_frame_texture :: proc() {
	texture_selections := cast(^[3]u32)r_renderer.texture_select_buffer->contents()

	previous_selection := texture_selections[(r_renderer.current_frame - 1) % R_NUM_FRAMES_IN_FLIGHT] - 1
	...
	texture_selections[r_renderer.current_frame % R_NUM_FRAMES_IN_FLIGHT] = new_selection + 1
}
...
r_do_main_renderpass :: proc() {
	r_renderer.argument_table->setAddress(
		r_renderer.texture_select_buffer->gpuAddress() +
			cast(MTL.GPUAddress)((r_renderer.current_frame % R_NUM_FRAMES_IN_FLIGHT) * size_of(u32)),
		2,
	)
